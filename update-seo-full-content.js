/**
 * Update Notion SEO package page - Delete children and add FULL content
 */
...
async function deleteAllChildren(pageId) {
  console.log('Fetching all children blocks...');
  const blocks = [];
  let hasMore = true;
  ...
  // Delete all blocks (but not the page itself)
  for (const block of blocks) {
    try {
      ...
      if (!error.message.includes('not found') && !error.message.includes('deleted')) {
        console.warn(`Warning: Could not delete block ${block.id}: ${error.message}`);
      }
    }
  }

  console.log('All children blocks deleted.');
}

async function addFullContent(pageId) {
  ...
  // Add blocks with delay
  for (const block of content) {
    await notion.blocks.children.append({
      block_id: pageId,
      children: [block],
    });
    await delay(200); // Delay to avoid rate limits
  }

  console.log('Full content added successfully.');
}
    ...
    if (searchResults.results.length > 0) {
      for (const result of searchResults.results) {
        if (result.properties.title && 
          ...
          await deleteAllChildren(result.id);
          
          // Add full content
          await addFullContent(result.id);
          ...
          console.log('─'.repeat(60));
          console.log('  ✅ Deleted all old content blocks');
